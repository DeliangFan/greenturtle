...
策略和参数完全复制 《趋势策略》
- 买如开仓：EMA(25, CLOSE[i]) > EMA(100, CLOSE[i]) && Close[i] > ATR(100, close[i])，i +1 天以 open 价买入开仓
- 卖出开仓：EMA(25, CLOSE[i]) < EMA(100, CLOSE[i]) && Close[i] < ATR(100, close[i])
- 卖出平仓：EMA(25, CLOSE[i]) < EMA(100, CLOSE[i])
- 买出平仓：EMA(25, CLOSE[i]) > EMA(100, CLOSE[i])
- 止损：3 * ATR
...
A：整体 CTA 产品近 5 年化平均收益约 15%。头部私募量化代表作品为

|      | 成立时间 | 年化 | 最大回撤 | 夏普率 | 最长不盈利(年) | 
| -------- | ----- | ---- | ----| --- | ---- |
| 黑翼CTA 8 号 |  6 | 12% | 8% | 1.16 | 2 |
...
|      | 成立时间 | 年化 | 最大回撤 | 夏普率 | 最长不盈利(年) | 
| -------- | ----- | ---- | ----| --- | ---- |
| 幻方沪深 300 指增 |  7 | 20% | 34% | 0.91 | 3.3 |
| 幻方中证 500 指增 | 8 | 20% | 34% | 0.92 | 3.3 |
| 黑翼中证 500 指增 | 6 | 19% | 32% | 0.82 | 3.5 |
